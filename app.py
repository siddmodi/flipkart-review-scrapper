...
class threadClass:
    ...
    def run(self):
        ...
        collection_name = self.scrapper_object.getReviewsToDisplay(expected_review=self.expected_review,
                                                                   searchString=self.searchString, username='modi1969',
                                                                   password='modi1969',
...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            mongoClient = MongoDBManagement(username='modi1969', password='modi1969')
...
def feedback():
    try:
        ...
        if collection_name is not None:
            scrapper_object = FlipkratScrapper(executable_path=ChromeDriverManager().install(),
                                               chrome_options=chrome_options)
            mongoClient = MongoDBManagement(username='Siddharth', password='Modi1969*')
